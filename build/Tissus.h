...
#include <build/Spring.h>

class Tissus {
public:
	float sizeParticule = 20;
	float poidsTissus = 0.5;
	Tissus(int sizeX, int sizeY, Point pos, float l0, float k, float v) : points(Array2D<Particle>(sizeX, sizeY)) {
	...
	Array2D<Particle>& getParticules() { return points; }
	std::vector<Spring>& getSprings() { return ressors; }
